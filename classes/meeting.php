...
class meeting {
    ...
    public function join(int $origin): string {
        ...
        $flas1 = $this->is_running();
        $flas2 = $this->instance->is_moderator();
         // If user is (administrator or moderator) and meeting has'nt started
         if(!$this->is_running() && $this->instance->is_moderator()) {
            logger::log_meeting_started_event($this->instance, $origin);
        }
