...
class logger {
    ...
    public const EVENT_SUMMARY = 'Summary';
     ...
     /** @var string The start meetings Join event - ELO */ 
...
EOF;
    ...
    protected static function log_moodle_event(instance $instance, string $type, array $options = []): void {
        if (!in_array($type, (array) \mod_bigbluebuttonbn\event\events::$events)) {
    ...
    const LOG_TO_STRING = [
        ...
        self::EVENT_MEETING_START => 'event_meeting_started', // Elo
    ];
     ...
     * @param int $origin
     */
    public static function log_meeting_started_event(instance $instance, int $origin): void {
        // Moodle event logger: Create an event for meeting joined.
        self::log_moodle_event($instance, events::$events['meeting_start']);
